...
def VAE(input_shape=[None, 784],
    ...
    shapes = copy.deepcopy(encoding['shapes'])
    shapes.reverse()
    n_filters = copy.deepcopy(n_filters)
    n_filters.reverse()
...
def VAEGAN(input_shape=[None, 784],
    ...
    shapes = copy.deepcopy(encoding['shapes'])
    shapes.reverse()
    n_filters_decoder = copy.deepcopy(n_filters)
...
def train_vaegan(files,
    ...
    ----------
    files : array
        list of files names to process
...
# def video_to_files(vid_filename, output_folder='video', output_filename='frame', w=160, h=90):
#     os.system('ffmpeg -i %s -r 60 -f' +
#                   ' image2 -s %dx%d %s/%s-%08d.jpg'%(
#                   vid_filename, w, h, output_folder, output_filename))
#     files = [os.path.join(ouput_folder, f) for f in os.listdir(output_folder) if
#                 f.startswith(output_filename) and f.endswith('.jpg')]
#     return files
